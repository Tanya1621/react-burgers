            ...
            <section className={style.login_page}>
                <form className={style.login_page__inputs} onSubmit={loginHandler}>
                    ...
                    <PasswordInput value={passwordValue} name={'password'} onChange={onPasswordChange}/>
                                   {/*;?extraClass={style.inputt}*/}
                    <Button type='primary' size='medium' htmlType={'submit'}>Войти</Button></form>
                <div>
                    <p className={`text text_type_main-small mb-4 ${style.paragraph}`}>Вы новый пользователь? <NavLink
                        to='/register' className={style.link}>Зарегистрироваться</NavLink></p>
                    <p className={`text text_type_main-small ${style.paragraph}`}>Забыли пароль? <NavLink
                        to='/forgot-password' className={style.link}>Восстановить пароль</NavLink></p>
                </div>
            </section>
        </>
    )
